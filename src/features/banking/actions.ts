...
export const initialisePaystackFunding = async (token: string, payload: {amount: number,
    ...
    return await sendApiRequest(
      'post',
      `${process.env.NEXT_PUBLIC_CENTINAIR_API_URL}/${VTU_URL}/initialise-paystack-funding`,
      payload,
      {
...
export const verifyPaystackFunding = async (token: string, payload: {
    ...
    return await sendApiRequest(
      'post',
      `${process.env.NEXT_PUBLIC_CENTINAIR_API_URL}/${VTU_URL}/verify-paystack-funding`,
      payload,
      {
        headers: { Authorization: `Bearer ${token}` },
        withCredentials: true
